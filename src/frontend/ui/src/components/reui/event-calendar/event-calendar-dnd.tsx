...
} from "@apesdb/ui/components/reui/event-calendar/event-calendar";
import {
  snapMinutes,
  toZoned,
  zonedStartOfDay,
} from "@apesdb/ui/components/reui/event-calendar/event-calendar-lib";
import type {
  EventCalendarProposedUpdate,
  EventCalendarSegment,
} from "@apesdb/ui/components/reui/event-calendar/event-calendar-types";
...
function beginGesture<TData>(config: BeginGestureConfig<TData>) {
  ...
  let copyModifier = kind === "move" && startEvent.altKey;
  ...
  const gestureCursor = kind.startsWith("resize")
    ? isBar
      ...
      : "ns-resize"
    : "grabbing";
  const setBodyDragging = (on: boolean, invalid = false, copy = false) => {
  ...
  const activate = () => {
    ...
    setBodyDragging(true, false, copyModifier);
    createCarry();
  };
  ...
  const applyProposal = (e: PointerEvent) => {
    ...
    if (!proposal) return;
    const copy = copyModifier;
    const key = `${proposal.start.getTime()}-${proposal.end.getTime()}-${proposal.allDay}-${proposal.resourceId ?? ""}-${copy}`;
    ...
    lastValid = valid;
    setBodyDragging(true, !valid, copy);
    internals.setDrag({
  ...
  const positionCarry = (e: PointerEvent) => {
    ...
    carryEl.toggleAttribute("data-copy", copyModifier);
  ...
  const onKeyDown = (e: KeyboardEvent) => {
    ...
    if (e.key === "Alt" && kind === "move" && active && !copyModifier) {
      copyModifier = true;
      applyProposal(lastPointer);
      positionCarry(lastPointer);
    }
  };
  ...
  const onPointerMove = (e: PointerEvent) => {
    ...
    if (e.pointerId !== pointerId) return;
    lastPointer = e;
    copyModifier = kind === "move" && e.altKey;
    if (!active) {
  ...
  const onPointerUp = (e: PointerEvent) => {
    ...
    const valid = settings.canDropEvent ? settings.canDropEvent(update) : true;
    if (!valid) return;
    const accepted = internals.applyProposedUpdate(update);
