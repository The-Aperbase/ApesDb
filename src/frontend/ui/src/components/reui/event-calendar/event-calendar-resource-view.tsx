...
import {
  ...
  wasRecentChipPress,
  wasRecentDrag,
} from "@apesdb/ui/components/reui/event-calendar/event-calendar-dnd";
import {
  EVENT_CALENDAR_GHOST,
  EventCalendarEvent,
} from "@apesdb/ui/components/reui/event-calendar/event-calendar-event";
import {
  flattenResources,
  getDayKey,
  getDayTotalMinutes,
...
import { ScrollArea } from "@apesdb/ui/lib/scroll-area";
...
function EventCalendarResourceColumn({
  ...
  const packed = useMemo(() => {
    const mine = segments.timed
      ...
      .map((segment) => Object.assign({}, segment) as EventCalendarSegment);
